  ...
  const handleTabChange = (tabId: string) => {
    ...
    useEffect(()=>{
      console.log("messge : ", messages);
       const totalUnseen = messages.reduce((count, msg) => {
        const unseenInThisConversation = msg.texts?.filter((t) => t.isSeen === false && t.sender != user?._id).length || 0;
    ...
    const handleSend = async (receiver: string, receiverName: string) => {
      if(user?._id && user.profile?.name && receiver && text){
        await sendMessage(user._id, user.profile.name, receiver, receiverName, text);
        setText("");
      }
    };
  ...
  return (
      ...
      <div className='bg-slate-400 py-2 absolute bottom-0 w-full flex justify-center gap-4'>
        <input type="text"
          ...
          placeholder='Message'
        />
        <button onClick={(e)=> {e.preventDefault(); handleSend(messages.find((msg) => msg.id === selectedConversation)?.participants?.[1] ?? '', messages.find((msg) => msg.id === selectedConversation)?.receiverName ?? '')}} className='bg-green-600 p-3 text-white rounded-full'>
          send
        </button>
      </div>
    </div>
        ...
        {messages && messages.length > 0 ? (
          <div className='space-y-4'>
            {messages.map((msg, index) => (
              <div key={index} onClick={()=> {setShowChat(true); setSelectedConversation(msg?.id); handleSeen(msg.id, user?._id) }} className='bg-white cursor-pointer rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow duration-200 border border-slate-200'>
                <div className='flex items-center gap-3 mb-3'>
                  <div className='bg-gradient-to-br from-blue-500 to-blue-600 p-2 w-10 h-10 rounded-full flex items-center justify-center shadow-md'>
                    <User className='h-5 w-5 text-white'/>
                  </div>
                  <div className='w-full'>
                    <div className='flex justify-between'>
                      <p className='font-semibold text-slate-800'>{msg.receiverName}</p>
                      <p className={`w-8 h-8 ${msg.texts.filter((m)=> m.isSeen == false && m.sender != user?._id ).length == 0 ? 'hidden' : 'flex'} justify-center items-center bg-green-600 text-white rounded-full mr-4`}>{msg.texts.filter((m)=> m.isSeen == false && m.sender != user?._id ).length}</p>
                    </div>
