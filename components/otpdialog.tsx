...
export default function OTPDialog({email}: OTPDialogProps) {
  ...
  const handleVerify = async() => {
    if (otp.every((d) => d !== "")) {
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/verifyotp`, {email,otp});
  ...
  const resetPassword = async() => {
    try {
        const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/resetpassword`, {email,newPassword});
        if(response.status == 200){
    ...
    <Dialog>
      <div className={`${showResetPassword ? 'hidden' : 'block'} sm:max-w-sm !rounded-xl p-6 border`}>
        ...
        <div className="flex flex-col gap-2">
          <Button className="w-full bg-black text-white" onClick={handleVerify}>
            Verify OTP
          </Button>
