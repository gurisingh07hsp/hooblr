  ...
  const handleCompanyInfoSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/companies/profile`,
        {
          company: {
  ...
  const handleDeleteCompnay = async (id: string) => {
    ...
    try {
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/companies/${id}`,
        { withCredentials: true }
      );
      if (response.status == 200) {
            ...
            <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-5">
              {user?.companies && user?.companies?.length > 0 ? (
                ...
                <button
                  ...
                  className="bg-[#6D47F1] text-white rounded-lg p-2 mx-auto"
                >
                  Create Company
                </button>
              )}
